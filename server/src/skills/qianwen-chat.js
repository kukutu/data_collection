...
export function buildQianwenChatPlan({
  app,
  ...
  if (!app?.packageName) {
    throw new Error('Qianwen chat skill requires an app packageName');
  }
  ...
  const steps = [
    { type: 'launch_app', packageName: app.packageName, label: '打开千问' },
    { type: 'wait', ms: 3000, label: '等待千问加载' },
    ...
    { type: 'start_capture', label: '开始采集千问对话内容' },
  ];
  ...
  steps.push({ type: 'complete', label: '千问英文聊天完成' });
  return steps;
}
...
function normalizeMessages(messages) {
  ...
  if (!normalized.length) {
    throw new Error('Qianwen chat skill requires at least one safe English message');
  }
  return normalized;
}
